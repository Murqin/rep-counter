...
describe('Counter Component', () => {
  it('displays current rep and round', () => {
    const { getByText } = render(Counter, { targetReps: 10, onOpenSettings: vi.fn() });
    expect(getByText('5')).toBeTruthy();
    expect(getByText('Round 2')).toBeTruthy();
  });

  it('calls incrementRep on click', async () => {
    const { getByTestId } = render(Counter, { targetReps: 10, onOpenSettings: vi.fn() });
  ...
  it('shows NEXT ROUND button when target is reached and autoAdvance is false', () => {
    const { getByText } = render(Counter, { targetReps: 5, onOpenSettings: vi.fn() });
  ...
  it('opens QuickAdjust when round or target text is clicked', async () => {
    const { getByText, queryByText } = render(Counter, { targetReps: 10, onOpenSettings: vi.fn() });
